...
namespace XFiles.Metadata
{
    public class MetadataGuesser
    {
        ...
        public async Task<MetadataMatch> ResolveAsync(string filePath, CancellationToken ct = default)
        {
            ...
            Log.Dbg("MetadataGuesser: filename parsed title={Title} artist={Artist} album={Album} track={Track}",
                filenameMeta.Title, filenameMeta.Artist, filenameMeta.Album, filenameMeta.TrackNumber);

            local.MergeFrom(filenameMeta);

            Log.Dbg("MetadataGuesser: after merge title={Title} artist={Artist} album={Album}",
            ...
            if (local.HasAlbumArt)
            {
                ...
                if (!string.IsNullOrEmpty(coverKey))
                {
                    ...
                    Log.Dbg("MetadataGuesser: stored embedded cover art for album key '{Key}'", coverKey);
                }
            }
            ...
            if (!string.IsNullOrWhiteSpace(local.Artist) || !string.IsNullOrWhiteSpace(local.Title))
            {
                ...
                Log.Dbg("MetadataGuesser: cache key='{Key}' artist='{Artist}' title='{Title}' album='{Album}'",
                    cacheKey, local.Artist, local.Title, local.Album);
                var cached = await _cache.GetAsync(cacheKey);
                if (cached != null)
                {
                    Log.Dbg("MetadataGuesser: cache hit — title={Title} artist={Artist} album={Album} art={HasArt}",
                    ...
                    Log.Dbg("MetadataGuesser: after cache merge — title={Title} artist={Artist} album={Album} art={HasArt}",
                        cached.Metadata.Title, cached.Metadata.Artist, cached.Metadata.Album, cached.Metadata.HasAlbumArt);
                    return cached;
                }
                ...
                if (!local.HasAlbumArt && _internetAvailable)
                {
                    ...
                    if (existingCover != null)
                    {
                        ...
                        Log.Dbg("MetadataGuesser: reusing cached cover art for album key '{Key}'", coverKey);
                    }
                }
            }
            ...
            if (_internetAvailable)
            {
                ...
                if (online != null)
                {
                    Log.Dbg("MetadataGuesser: Deezer result — usable={Usable} score={Score:F2} title={Title} artist={Artist} album={Album} coverUrl={HasUrl}",
                ...
                {
                    Log.Dbg("MetadataGuesser: Deezer returned null, trying MusicBrainz fallback");
                }

                if (online == null || !online.IsUsable)
                {
                    Log.Dbg("MetadataGuesser: trying MusicBrainz fallback");
                    online = await _musicBrainz.SearchRecordingAsync(
                        local.Artist, local.Title, local.Album, ct);

                    if (online != null)
                    {
                        Log.Dbg("MetadataGuesser: MusicBrainz result — usable={Usable} score={Score:F2} title={Title} artist={Artist} album={Album}",
                        ...
                        {
                            Log.Dbg("MetadataGuesser: fetching cover art for release {Release}", online.ReleaseMbid);
                            ...
                            if (coverArt != null && coverArt.Length > 0)
                            {
                                ...
                                Log.Dbg("MetadataGuesser: cover art loaded ({Size} bytes)", coverArt.Length);
                            }
                        }
                    }
                }
                ...
                if (online != null && online.IsUsable)
                {
                    ...
                    Log.Dbg("MetadataGuesser: after online merge — title={Title} artist={Artist} album={Album} art={HasArt}",
                    ...
                    Log.Dbg("MetadataGuesser: cover art check — hasArt={HasArt} coverUrl='{Url}' shouldFetch={ShouldFetch}",
                    ...
                    if (shouldFetchArt)
                    {
                        ...
                        if (!string.IsNullOrEmpty(cachedCoverUrl) && cachedCoverUrl == online.CoverArtUrl)
                        {
                            ...
                            if (existingArt != null && existingArt.Length > 0)
                            {
                                ...
                                Log.Dbg("MetadataGuesser: reused cached cover art ({Size} bytes)", existingArt.Length);
                    ...
                    if (shouldFetchArt)
                    {
                        Log.Dbg("MetadataGuesser: fetching cover art from Deezer URL {Url}", online.CoverArtUrl);
                        ...
                        {
                            Log.Dbg("MetadataGuesser: no cover art available");
                        }
                    }
                    ...
                    {
                        string cacheKey = MetadataCache.BuildCacheKey(local.Artist, local.Title, local.Album);
                        Log.Dbg("MetadataGuesser: caching result key='{Key}' art={HasArt}", cacheKey, online.Metadata.HasAlbumArt);
                ...
                Log.Dbg("MetadataGuesser: no usable online match for {Title}", local.Title);
            }
            else
            {
                Log.Dbg("MetadataGuesser: internet not available, using local data only");
            }
