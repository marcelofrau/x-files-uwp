...
namespace XFiles.Metadata
{
    public class MetadataGuesser
    {
        ...
        {
            Log.Debug("MetadataGuesser: resolving {Path}", filePath);

            var id3Tag = await Task.Run(() => FileSystem.Id3Tag.ReadFromFile(filePath), ct);
            var local = TrackMetadata.FromId3Tag(id3Tag, filePath);

            Log.Debug("MetadataGuesser: local ID3 title={Title} artist={Artist} album={Album} genre={Genre} year={Year} track={Track} art={HasArt}",
            ...
            Log.Debug("MetadataGuesser: filename parsed title={Title} artist={Artist} album={Album} track={Track}",
                filenameMeta.Title, filenameMeta.Artist, filenameMeta.Album, filenameMeta.TrackNumber);

            local.MergeFrom(filenameMeta);

            Log.Debug("MetadataGuesser: after merge title={Title} artist={Artist} album={Album}",
            ...
            if (local.HasAlbumArt)
            {
                ...
                if (!string.IsNullOrEmpty(coverKey))
                {
                    ...
                    Log.Debug("MetadataGuesser: stored embedded cover art for album key '{Key}'", coverKey);
                }
            }
            ...
            if (!string.IsNullOrWhiteSpace(local.Artist) || !string.IsNullOrWhiteSpace(local.Title))
            {
                ...
                Log.Debug("MetadataGuesser: cache key='{Key}' artist='{Artist}' title='{Title}' album='{Album}'",
                    cacheKey, local.Artist, local.Title, local.Album);
                var cached = await _cache.GetAsync(cacheKey);
                if (cached != null)
                {
                    Log.Debug("MetadataGuesser: cache hit — title={Title} artist={Artist} album={Album} art={HasArt}",
                    ...
                    Log.Debug("MetadataGuesser: after cache merge — title={Title} artist={Artist} album={Album} art={HasArt}",
                        cached.Metadata.Title, cached.Metadata.Artist, cached.Metadata.Album, cached.Metadata.HasAlbumArt);
                    return cached;
                }
                ...
                if (!local.HasAlbumArt && _internetAvailable)
                {
                    ...
                    if (existingCover != null)
                    {
                        ...
                        Log.Debug("MetadataGuesser: reusing cached cover art for album key '{Key}'", coverKey);
                    }
                }
            }
            ...
            if (_internetAvailable)
            {
                ...
                if (online != null)
                {
                    Log.Debug("MetadataGuesser: Deezer result — usable={Usable} score={Score:F2} title={Title} artist={Artist} album={Album} coverUrl={HasUrl}",
                ...
                {
                    Log.Debug("MetadataGuesser: Deezer returned null, trying MusicBrainz fallback");
                }

                if (online == null || !online.IsUsable)
                {
                    Log.Debug("MetadataGuesser: trying MusicBrainz fallback");
                    online = await _musicBrainz.SearchRecordingAsync(
                        local.Artist, local.Title, local.Album, ct);

                    if (online != null)
                    {
                        Log.Debug("MetadataGuesser: MusicBrainz result — usable={Usable} score={Score:F2} title={Title} artist={Artist} album={Album}",
                        ...
                        {
                            Log.Debug("MetadataGuesser: fetching cover art for release {Release}", online.ReleaseMbid);
                            ...
                            if (coverArt != null && coverArt.Length > 0)
                            {
                                ...
                                Log.Debug("MetadataGuesser: cover art loaded ({Size} bytes)", coverArt.Length);
                            }
                        }
                    }
                }
                ...
                if (online != null && online.IsUsable)
                {
                    ...
                    Log.Debug("MetadataGuesser: after online merge — title={Title} artist={Artist} album={Album} art={HasArt}",
                    ...
                    Log.Debug("MetadataGuesser: cover art check — hasArt={HasArt} coverUrl='{Url}' shouldFetch={ShouldFetch}",
                    ...
                    if (shouldFetchArt)
                    {
                        ...
                        if (!string.IsNullOrEmpty(cachedCoverUrl) && cachedCoverUrl == online.CoverArtUrl)
                        {
                            ...
                            if (existingArt != null && existingArt.Length > 0)
                            {
                                ...
                                Log.Debug("MetadataGuesser: reused cached cover art ({Size} bytes)", existingArt.Length);
                    ...
                    if (shouldFetchArt)
                    {
                        Log.Debug("MetadataGuesser: fetching cover art from Deezer URL {Url}", online.CoverArtUrl);
                        ...
                        {
                            Log.Debug("MetadataGuesser: no cover art available");
                        }
                    }
                    ...
                    {
                        string cacheKey = MetadataCache.BuildCacheKey(local.Artist, local.Title, local.Album);
                        Log.Debug("MetadataGuesser: caching result key='{Key}' art={HasArt}", cacheKey, online.Metadata.HasAlbumArt);
                ...
                Log.Debug("MetadataGuesser: no usable online match for {Title}", local.Title);
            }
            else
            {
                Log.Debug("MetadataGuesser: internet not available, using local data only");
            }
